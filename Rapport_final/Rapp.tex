...
Pour la compression, on parcoure le fichier lettre par lettre et pour chaque lettre rencontrée on la map dans une \verb+HasMap+ associé à sa fréquence qui sera incrémenté a chaque fois que la lettre est de nouveau rencontrée.

Ensuite un \verb+HuffmanTree+ est crée par lettre associé avec sa fréquence et stocké en fonction de la valeur de celle-ci dans une file de priorité.

Grâce à la fonction mergeAll de la classe \verb+HuffmanBTRee+ on va crée notre arbre final de Hufman correspondant au fichier qu'on souhaite compressé.

Cette arbre sera mis sous forme binaire, chaque nœud correspondra à un 0 et chaque feuille à un 1 suivit de la valeur du caractère en bits. Il sera ensuite incorporé au début du fichier compresser afin de pouvoir le décompresser plus tard.
